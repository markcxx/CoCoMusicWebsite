...
// 企业版系统检测
function getEnterpriseSystemInfo(
  version: string,
): EnterpriseSystemInfo[] | null {
  if (isMobileDevice()) {
...
export function useEnterpriseVersionData() {
  ...
  const [versionData, setVersionData] = useState<EnterpriseVersionData | null>(
    null,
  );
  const [systemInfo, setSystemInfo] = useState<EnterpriseSystemInfo[] | null>(
    null,
  );
  const [downloadUrls, setDownloadUrls] =
    useState<EnterpriseDownloadUrls | null>(null);
    ...
    const fetchVersionData = async () => {
      try {
        ...
        const versionData: EnterpriseVersionData = {
          ...
          assets:
            data.assets?.filter(
              (asset: EnterpriseAsset) => asset.type === "attach",
            ) || [],
        };
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Failed to fetch enterprise version data",
        );
        setLoading(false);
      }
    };
