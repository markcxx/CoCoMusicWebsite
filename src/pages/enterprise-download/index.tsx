...
const GITCODE_RELEASE_URL =
  "https://gitcode.com/CherryHQ/cherry-studio-enterprise/releases";
...
const DownloadButton: FC<DownloadButtonProps> = ({ info }) => {
  ...
  const getButtonLabel = () => {
    ...
    return t("download_page.download_with_arch", {
      arch: info.arch.toUpperCase(),
    });
  };
  ...
  return (
    <div className="enterprise-download-buttons">
      <p className="fallback-notice">
        {t("enterprise_page.beta.download.fallback_notice") ||
          "无法获取版本信息，请前往 GitCode 下载"}
      </p>
      <div className="button-group">
        ...
        >
          ...
          <span>
            {t("enterprise_page.beta.download.goto_gitcode") ||
              "前往 GitCode 下载"}
          </span>
        </a>
      </div>
      ...
      <ul id="download-list">
        {Object.values(downloadUrls).map((group) => (
          <div key={group.title}>
            <h3>{group.title}</h3>
            {group.items.map(
              (item: { name: string; url: string; desc: string }) => (
                <li key={item.name}>
                  <button
...
const EnterpriseDownloadPage: FC = () => {
  ...
  const { loading, error, versionData, systemInfo, downloadUrls } =
    useEnterpriseVersionData();
  ...
  const renderDownloadContent = () => {
    ...
    return (
      <DownloadButtons
        ...
        onOtherVersionsClick={scrollToOtherDownloads}
      />
    );
  };
            ...
            <div className="beta-card">
              ...
              <div className="card-content">
                <ul>
                  ...
                  <li>
                    <strong>{t("enterprise_page.beta.demo.account")}</strong>
                    admin
                  </li>
                  <li>
                    <strong>{t("enterprise_page.beta.demo.password")}</strong>
                    password
                  </li>
                </ul>
              </div>
            ...
            <div className="beta-card">
              ...
              <div className="card-content">
                <ul className="server-info">
                  <li>
                    {t("enterprise_page.beta.download.server_url")}{" "}
                    https://api.demo.cherry-ai.com
                  </li>
                  <li>
                    <strong>{t("enterprise_page.beta.demo.account")}</strong>
                    user
