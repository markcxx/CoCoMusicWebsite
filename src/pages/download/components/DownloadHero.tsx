...
const DownloadHero: FC<DownloadHeroProps> = ({ versionData, loading, onViewAllClick }) => {
  const { t } = useTranslation()

  // 尝试找到 Windows 安装包，如果找不到则使用 Release 页面链接
  const windowsSetup = versionData?.assets.find((a) => /setup.*\.exe$/i.test(a.name))?.browser_download_url
  const downloadUrl =
    windowsSetup ||
    (versionData?.version
        ...
        <div className="mx-auto max-w-2xl text-center">
          <div className="mb-8 flex justify-center">
            <div className="relative rounded-full bg-white px-4 py-1.5 text-sm leading-6 text-gray-600 ring-1 ring-gray-900/10 hover:ring-gray-900/20">
              <span className="font-semibold text-[var(--theme-color)]">{t('download_page.recommended_version')}</span>
          ...
          <p className="mt-6 text-lg leading-8 text-gray-600">{t('download_page.hero_subtitle')}</p>
          ...
          <div className="mt-10 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
            ...
            <a
              ...
              <div className="text-left">
                <div className="text-lg leading-none font-bold">{t('download_page.download_for_windows')}</div>
