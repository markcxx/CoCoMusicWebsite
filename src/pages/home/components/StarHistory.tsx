...
const StarHistory: FC = () => {
  ...
  useEffect(() => {
    const API_URL =
      "https://data1.cherry-ai.com:48443/items/github_cherrystudio";
    let chart: any = null;
    ...
    function initDatePickers(data: any) {
      const dates = data.data.cherrystudio_data.starHistory.map(
        (item: any) => item.date,
      );
      ...
      const filteredData = {
        ...
        data: {
          ...
          cherrystudio_data: {
            ...
            starHistory: fullData.data.cherrystudio_data.starHistory.filter(
              (item: any) => {
      ...
      const option = {
        ...
        toolbox: {
          feature: {
            dataView: {
              ...
              readOnly: false,
              lang: [
                t("star_history.data_view"),
                t("star_history.close"),
                t("star_history.refresh"),
              ],
            },
            restore: {
              show: true,
        ...
        xAxis: {
          ...
          axisLabel: {
            formatter: function (value: any) {
              ...
              if (isMobile) {
                return yearSuffix
                  ? `${month}${monthSuffix}${day}`
                  : `${month}/${day}`;
              }
              if (month === 1 && day === 1) {
                return yearSuffix
                  ? `${date.getFullYear()}${yearSuffix}`
                  : `${date.getFullYear()}`;
              }
              return yearSuffix
                ? `${month}${monthSuffix}${day}${daySuffix}`
                : `${month}/${day}`;
            },
        ...
        dataZoom: [
          ...
          },
          {
            ...
            handleIcon:
              "path://M-9.5,0a9.5,9.5,0,1,1,19,0a9.5,9.5,0,1,1,-19,0Z",
            handleSize: "16",
            handleStyle: {
              color: "#fff",
              borderColor: "#FF6262",
    ...
    function renderContributors(data: any) {
      ...
      const contributors =
        data.data.cherrystudio_data.contributors.contributors;
      ...
      const showCount =
        data.data.show_contributor_count || sortedContributors.length;
      ...
      const titleElement = document.querySelector(
        ".contributors-section .heading_title",
      );
      if (titleElement) {
          ...
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              ...
              animateValue(
                contributorsCount,
                0,
                contributorsStats.total_contributors,
                2000,
              );
